...
class OpenAIRealtime {
    ...
    uso = { audioIn: 0, textIn: 0, cachedAudioIn: 0, cachedTextIn: 0, audioOut: 0, textOut: 0 };
    ...
    async abrirConexion() {
        try {
            ...
            if (MODELO_PREFERIDO === MODELO_FALLBACK)
                throw err;
    ...
    // ============================================================================
    configurarSesion(prompt, tools = [], voz, velocidad) {
        ...
        switch (msg.type) {
            ...
            case "conversation.item.added":
                if (msg.item?.role === "user" &&
                    msg.item?.id &&
                    this.onItemCreated &&
                    !msg.item?.content?.some((c) => (0, normalizar_hora_1.esAnotacionHoraria)(c?.text))) {
            ...
            case "conversation.item.input_audio_transcription.completed":
                ...
                if (msg.transcript && this.ws && this.conectado) {
                    const hora = (0, normalizar_hora_1.ultimaHoraExplicita)(msg.transcript);
                    if (hora)
                        this.horaExplicitaTurno = { hora, detectadaEl: Date.now() };
                    const anotacion = (0, normalizar_hora_1.anotacionHorariaParaAgente)(msg.transcript);
                    if (anotacion) {
            ...
            case "response.function_call_arguments.done":
                if (this.funcionActual && this.onFunctionCall) {
                    ...
                    if (this.horaExplicitaTurno && Date.now() - this.horaExplicitaTurno.detectadaEl < 120_000) {
                        args = (0, normalizar_hora_1.normalizarArgumentosConHora)(args, this.horaExplicitaTurno.hora);
                    }
    ...
    programarRespuestaUsuario(delayMs = DEBOUNCE_RESPUESTA_MS) {
        this.cancelarDebounceRespuesta();
    ...
    intentarRespuestaUsuarioProgramada() {
    ...
    setOnEspera(callback) { this.onEspera = callback; }
