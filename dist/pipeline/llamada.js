...
function buildSystemPrompt(cfg) {
    ...
    const habitacionesTexto = vertical === "hospedaje" && itemsHospedaje.length > 0
        ? itemsHospedaje.map((h) => `- ${h.nombre}${h.capacidad ? ` (capacidad ${h.capacidad})` : ""} — ${formatearMoneda(h.precio)}${h.unidad ? ` ${h.unidad}` : " por noche"}${h.descripcion ? ` — ${h.descripcion}` : ""}`).join("\n")
        : null;
    ...
    const habitacionesConId = itemsHospedaje.length > 0
        ? itemsHospedaje.map((h) => `- ${h.nombre} [ID:${h.id}]${h.capacidad ? ` (capacidad ${h.capacidad})` : ""} — ${formatearMoneda(h.precio)}${h.unidad ? ` ${h.unidad}` : " por noche"}${h.descripcion ? ` — ${h.descripcion}` : ""}`).join("\n")
        : null;
...
function odinAuth() {
    return config_1.config.voiceServerSecret
...
async function fetchConfigConRetry(url, timeoutMs = 10000) {
    for (let intento = 1; intento <= 2; intento++) {
        try {
            ...
            if (resp.status === 404)
                return null; // no existe → no reintentar
        }
        catch (err) {
...
class PipelineLlamada {
    ...
    callerNumber;
    turnos = 0;
    constructor(ws, negocioId, configNegocio, callerNumber = "", numeroTwilio = "", callSid = "") {
        this.ws = ws;
        ...
        this.realtime = new realtime_1.OpenAIRealtime(prompt, herramientas, configNegocio.voz || "marin");
    }
    esTranscripcionValida(texto) {
        ...
        if (t.length < 4)
            return false;
        if (tLower.includes("www.") || tLower.includes("http") || tLower.includes(".com") || tLower.includes(".org"))
            return false;
        if (/^[\s.,!?¿¡0-9\-]+$/.test(t))
            return false;
        const ruidoExacto = [
        ...
        if (ruidoExacto.includes(tLower))
            return false;
    ...
    async colgarTwilioCall() {
        ...
        try {
            const client = (0, twilio_1.default)(config_1.config.twilioAccountSid, config_1.config.twilioAuthToken);
    ...
    async manejarFuncion(nombre, args, callId) {
        const odinUrl = config_1.config.odinAppUrl;
        ...
        try {
            switch (nombre) {
                ...
                case "cancelar_cita": {
                    ...
                    if (!resp.ok)
                        return { ok: false, mensaje: "No pude cancelar la cita. Por favor contacta al negocio directamente." };
                ...
                case "escalar_humano": {
                    ...
                    if (!respEscalar.ok) {
                        ...
                        const data = (await respEscalar.json().catch(() => ({})));
...
<Response>
  <Dial${callerIdAttr} timeout="25" answerOnBridge="true">${transferTo}</Dial>
        ...
        this.realtime.setOnTranscript((texto, role, itemId) => {
            if (role === "user") {
                ...
                if (!this.esTranscripcionValida(texto)) {
                    if (idx !== -1)
                        this.historialOrdenado.splice(idx, 1);
    ...
    async iniciar() {
        ...
        const vozRegistrada = this.numeroTwilio ? (0, registro_voz_1.obtenerVozPorNumero)(this.numeroTwilio) : null;
        if (vozRegistrada) {
        ...
        if (this.negocioId)
            params.set("negocioId", this.negocioId);
        if (this.numeroTwilio)
            params.set("numeroTwilio", this.numeroTwilio);
        ...
        if (configRapida) {
            ...
            if (configRapida.negocioId)
                this.negocioId = configRapida.negocioId;
            if (vozRegistrada)
                this.configNegocio.voz = vozRegistrada;
            ...
            fetchPromise.then((configCompleta) => {
                ...
                if (configCompleta.negocioId)
                    this.negocioId = configCompleta.negocioId;
    ...
    async finalizarLlamada() {
        ...
        if (historial.length > 0)
            console.log("[PIPELINE] Historial:\n" + transcripcion);
        ...
        try {
            const resp = await fetch(`${config_1.config.odinAppUrl}/api/webhooks/voice`, {
                method: "POST",
                headers: { "Content-Type": "application/json", ...odinAuth() },
                body: JSON.stringify({
...
exports.PipelineLlamada = PipelineLlamada;
