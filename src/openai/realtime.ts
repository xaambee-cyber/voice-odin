...
function fraseEspera(nombre: string): string {
  switch (nombre) {
    case "agendar_cita":
    case "reagendar_cita":
...
export class OpenAIRealtime {
  ...
  private fillersReproducidos: number = 0;
  private resultadoListoParaHablar: boolean = false;
  ...
  private reproducirFraseEspera(nombreFuncion: string) {
    if (!this.ws || !this.conectado) return;
