 ...
 */
function propiedadDatosDueno(
  campos: CampoMotorVoz[] | undefined,
  ...
  return {
    type: "object",
    description:
      "Datos que este negocio pide SIEMPRE para esta operación. Las LLAVES son los IDs de campo, NUNCA las etiquetas." +
      (obligatorios.length > 0
        ? ` Antes de llamar a esta función recolecta conversando: ${obligatorios.join(", ")}.`
        : ""),
    properties: props,
 ...
 */
function herramientasDeCicloVida(operaciones: OperacionClienteVoz[]): HerramientaVoz[] {
...
export function construirHerramientas(cfg: ConfigNegocio): HerramientaVoz[] {
  const herramientas: HerramientaVoz[] = [];
      ...
      parameters: {
        ...
        properties: {
          ...
          camposAgenda: {
            type: "object",
            description:
              "Datos adicionales que pide el servicio elegido. Las LLAVES deben ser los IDs de campo (ej. \"c1\", \"c2\"), NUNCA las etiquetas largas; los valores son lo que dijo el cliente. Inclúyelo SOLO si el servicio tiene campos adicionales; OMÍTELO por completo si el servicio no tiene ninguno." +
...
`
  ...
  const NOMBRE_OPERACION_DUENO: Record<string, string> = {
    cita: "agendar una cita",
    reserva: "hacer una reserva",
    pedido: "registrar un pedido",
  };
  const LLAVE_DUENO: Record<string, string> = {
    cita: "camposAgenda",
    reserva: "camposMotor",
    pedido: "camposMotor",
  };
  const bloqueDatosDueno = (() => {
    const partes = (["cita", "reserva", "pedido"] as const)
      ...
      .map((t) => {
        ...
        const lista = campos
          .map(
            (c) =>
              `${c.label} [campo:${c.id}, ${c.requerido ? "OBLIGATORIO" : "opcional"}]${c.opciones?.length ? ` — opciones: ${c.opciones.join(", ")}` : ""}${c.ayuda ? ` — ${c.ayuda}` : ""}`,
          )
          .join("; ");
 ...
 */
function limpiarCamposDueno(raw: any): Record<string, string> | undefined {
 ...
 */
function limpiarReferencia(raw: any): string {
...
<Response>
  <Dial${callerIdAttr} timeout="15" answerOnBridge="true"><Number statusCallback="${cbEstado}" statusCallbackEvent="completed" statusCallbackMethod="POST">${transferTo}</Number></Dial>
        ...
        case "cancelar_operacion": {
          ...
          const dataOp: any = await respOp.json().catch(() => ({}));
          if (!respOp.ok) {
